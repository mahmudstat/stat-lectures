...
Complementary event
: Non-occurrence of an event. $P(\bar A) = 1 - P(A)$, where $\bar A$ or $A'$ or $A^c$ is called complement of A. 
...
|   x  |   0  |   1  |   2  | 3 |   4  |
|:----:|:----:|:----:|:----:|:-:|:----:|
| P(x) | 0.15 | 0.05 | 0.40 | k | 0.30 |

**Hint:** $\sum P(x) = 1$

### Find k from the function and then the mentioned probabilities.
...
$$\frac{1+k}{14}+\frac{2+k}{14}+\cdots = 1$$

Now, these are called discrete distributions, since values of x are specific and isolated. The distributions involving a discrete random variable is called a probability (mass) function (pmf), and are denoted as P(x).  

Another example is shown below.

### Solve the problems using the probability function. 

$$P(x) = \frac{2x+k}{56}; x=-3,-2,-1,0,1,2,3$$

a. Find k
b. Putting k in the function, verify this to be a pmf. 
c. $P(X>2)$
d. $P(-3 \le x < 0)$
e. $P(X>4)$

**Hint for b:** Check two things

i. Whether $0 \le P(x) \le 1)$ holds.
ii. $\sum (P(x) = 1$
